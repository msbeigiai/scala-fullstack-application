...
import com.msbeigi.core.domain.{Host, Index, Port, Scheme, ShardReplicas, Shards}
...
object EsConfig {
  def esConfig[F[_] : Async]: ConfigValue[F, EsConfig] = (
    ...
    EsConfig(Index(index), Host(host), Port(port), Scheme(scheme),
      Shards(numberOfShards), ShardReplicas(replicas))
  )
